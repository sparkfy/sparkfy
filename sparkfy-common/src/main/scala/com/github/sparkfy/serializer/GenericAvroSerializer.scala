...
import com.github.sparkfy.codec.CompressionCodec
 ...
 */
class GenericAvroSerializer(conf: Map[String, String])
  extends KSerializer[GenericRecord] {

  /**
   * a map where the keys are unique IDs for Avro schemas and the values are the
   * string representation of the Avro schema, used to decrease the amount of data
   * that needs to be serialized.
   */
  private val schemas: Map[Long, String] = conf.getAvroSchema
  ...
  private lazy val codec = CompressionCodec.createCodec(conf)
